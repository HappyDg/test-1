...
/******************************************************************************
 * Function:    OSInitAlarmAction
 *
 * Description: initializes alarm action
 *
 * Returns:     none
 *
 ******************************************************************************/
OSINLINE void
OSInitAlarmAction (OSWORD almId)
{
    OSALMARRAY[almId].action.taskIndex = OsAlarmsCfg[almId].taskIndex;
    /* for BCC1 events are not supported, action is only activate task */

    #if defined(OSALMSETEVENT)      /* events are supported if necessary */
    OSALMARRAY[almId].action.event = OsAlarmsCfg[almId].event;      /* if event==0 then alarm activates task
                                                                       else it sets event for this task */
    #elif defined(OSSCTSETEVENT)
    OSALMARRAY[almId].action.event = 0U; /* alarm event field is not used in this configuration */
...
OS_GetAlarmBase (AlarmType almId, AlarmBaseRefType info)
{
    ...
    ctrInd = OsAlarmsCfg[almInd].ctrIndex;
