...
OSInitApplications (void)
{
    ...
    for (i = 0U; i < OSNAPPS; i++)
    {
        ...
        OsAppTable[i].startup = OsAppCfgTable[i].startup;
        #endif
        #if defined(OSAPPHOOKSHUTDOWN)
        OsAppTable[i].shutdown = OsAppCfgTable[i].shutdown;
        #endif
        #if defined(OSAPPHOOKERROR)
        OsAppTable[i].error = OsAppCfgTable[i].error;
        #endif
        OsAppTable[i].tasks = OsAppCfgTable[i].tasks;
        #if defined(OSEXTPRIORS)
        OsAppTable[i].tasks2 = OsAppCfgTable[i].tasks2;
        #endif
        #if defined(OSHAS_RESTARTTASK)
        OsAppTable[i].restartTask = OsAppCfgTable[i].restartTask;
        #endif

        OSSETAPPSTATE(i, APPLICATION_ACCESSIBLE);
...
OS_CheckObjectAccess (ApplicationType appId, ObjectTypeType objectType, OSObjectType objectId)
{
    ...
    switch (objectType)
    {
        ...
        case OBJECT_ISR:
            if (objIndex < OSNISR)
            {
                appMask = OSGIVENAPPMASK (OsIsrCfg[objIndex].appId);
            }
            #if defined(OSHOOKERROR_EXT)
            else
            {
...
OS_CheckObjectOwnership (ObjectTypeType objectType, OSObjectType objectId)
{
    ...
    {
    case OBJECT_TASK:
        appId = (objIndex < OSNTSKS) ? OS_MKOBJID (OBJECT_APPLICATION, OsTaskCfgTable[objIndex].appId) : INVALID_OSAPPLICATION;
        break;

    #if (OSNISR > 0U)
    case OBJECT_ISR:
        appId = (objIndex < OSNISR) ? OS_MKOBJID(OBJECT_APPLICATION, OsIsrCfg[objIndex].appId) : INVALID_OSAPPLICATION;
        break;
    #endif

    #if (OSNUSERALMS > 0U)
    case OBJECT_ALARM:
        appId = (objIndex < OSNUSERALMS) ? OS_MKOBJID (OBJECT_APPLICATION, OsAlarmsCfg[objIndex].appId) : INVALID_OSAPPLICATION;
        break;
    #endif

    #if defined(OSCOUNTER)
    case OBJECT_COUNTER:
        appId = (objIndex < OSNCTRS) ? OS_MKOBJID (OBJECT_APPLICATION, OsCountersCfg[objIndex].appId) : INVALID_OSAPPLICATION;
        break;
    #endif

    #if defined(OSSCHEDULETABLES)
    case OBJECT_SCHEDULETABLE:
        appId = (objIndex < OSNSCTS) ? OS_MKOBJID (OBJECT_APPLICATION, OsScheduleTablesCfg[objIndex].appId) : INVALID_OSAPPLICATION;
        break;
    #endif
...
OSTerminateApplication (OSAPPLICATIONTYPE appInd, RestartType RestartOption)
{
    ...
    #if defined(OSHAS_RESTARTTASK) || (OSNTRUSTEDFUNCTIONS > 0U)
    OSAPPCB *app;
    #endif
    #if defined(OSCOUNTER)
    OSWORD i;
    ...
    {   /* set Task ready as in ActivateTask   */
        ...
        OSSETAPPSTATE(appInd, APPLICATION_RESTARTING);
    }
    else
    {
        OSSETAPPSTATE(appInd, APPLICATION_TERMINATED);
    }
    #else
    OSSETAPPSTATE(appInd, APPLICATION_TERMINATED);
...
OS_AllowAccess (void)
{
    ...
    OSSETAPPSTATE(appId, APPLICATION_ACCESSIBLE);

    for (i = 0U; i < OSNTSKS; i++)
    {
        if (appId == OsTaskCfgTable[i].appId)
        {
    ...
    {
        if (appId == OsScheduleTablesCfg[i].appId)
        {
    ...
    for (i = 0U; i < OSNCTRS; i++)
    {
        if (appId == OsCountersCfg[i].appId)
        {
...
OS_GetApplicationState (ApplicationType appId, ApplicationStateRefType stateRef)
{
    ...
    *stateRef = OSGETAPPSTATE( OS_GETOBJIND (appId) );
