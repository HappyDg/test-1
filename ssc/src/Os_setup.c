...
#if defined(OSMULTICORE) && defined(OSSBSSCOPY)
...
#else  /* defined(OSMULTICORE) && defined(OSSBSSCOPY) */

#define  OS_START_SEC_VAR_FAST_UNSPECIFIED
#include "Os_sections.h"
OSNEAR OSBYTE OsStarted;    /* boolean variable, it shall be OSFALSE (0) before StartOS() call  */
...
#endif /* defined(OSMULTICORE) && defined(OSSBSSCOPY) */
...
OSUSE_FRAME void StartOS (AppModeType mode)
{
    ...
    OsInitialMSR = 0x0u;                        /* initial MSR, used for Tasks start, interrupts are enabled */
    #endif
...
OSUSE_FRAME void OSShutdownOS (StatusType error)
{
    ...
    OS_SHUTDOWN_INTERRUPTS ();            /* disable ALL interrupts    */
    OSMSYNC ();
...
OS_GetActiveApplicationMode (void)
{
    ...
    #endif /* defined(OSEXTSTATUS) && defined(OSHOOKERROR_EXT) */
