...
OSDECLAREVAR (extern OSNEAR OSTSKCBPTR, OS_TaskListHead_); /**< The head of the task list */
#define OS_TaskListHead OSUSEVAR(OS_TaskListHead_)
...
/***************************************************************************
 * Macro:    OSGetTaskListHead
 * Description: get the head in the task list
 **************************************************************************/
OSINLINE OSTSKCBPTR OSGetTaskListHead(void)
{

        OSGETCOREIND
        return OS_TaskListHead;
}


/***************************************************************************
 * Macro:    OSSetTaskListHead
 * Description: set the head in the task list
 **************************************************************************/
OSINLINE void OSSetTaskListHead(OSTSKCBPTR ptr)
{
...
#define OSSAVECONTEXTADDR( task )            \
    (task)->prev = OSGetTaskListHead();      \
    OSSetTaskListHead(task)

/***************************************************************************
 * Macro:    OSLOADCONTEXTADDR
 * Description: Loads the content of the SPRG1 from Task context.
 **************************************************************************/
#define OSLOADCONTEXTADDR( task ) OSSetTaskListHead(task->prev)
...
{                                           \
    ...
    OSMSYNC(); task = OSGetTaskListHead();  \
...
extern void OSTaskTerminateDispatch (void);
         ...
         #pragma diag_suppress 667
          #if defined(OSTSKFPU)
          OSASM OSSIGNEDDWORD OSSetJmp (OSJMP_BUF jmp_buf, OSBYTE TSKUSEFPU);
          OSASM OSSIGNEDDWORD OSLongJmp (OSJMP_BUF jmp_buf, OSBYTE TSKUSEFPU);
          #else
          extern OSASM OSSIGNEDDWORD OSSetJmp  ( OSJMP_BUF );
          extern OSASM OSSIGNEDDWORD OSLongJmp ( OSJMP_BUF );
          #endif /* OSTSKFPU */
         #pragma diag_default 667
        #else
         #if defined(OSTSKFPU)
         OSSIGNEDDWORD OSSetJmp (OSJMP_BUF jmp_buf, OSBYTE TSKUSEFPU);
